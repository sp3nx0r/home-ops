...
- The cache is an optimization only. VolSync recreates the controller-owned
  ...
  its cache.
- VolSync sources start on the hour. The daily scrub runs at 00:30 in
  `America/Chicago`, leaving a buffer after the preceding scheduled source run.
